...
const CreateInitialSetup = async ({
  ...
  email,
  password
}: Request): Promise<{ id: number; name: string; email: string; profile: string }> => {
  const usersCount = await User.count();
  if (usersCount > 0) {
    throw new AppError("Setup already completed", 403);
  }

  // Default settings (mirrors the seed) so signup/login work out of the box.
  await Setting.findOrCreate({
    where: { key: "userCreation" },
    defaults: { key: "userCreation", value: "enabled" } as any
  });
